class Lead < ApplicationRecord
  ...
  def self.next(admin_email=nil)
    if admin_email == "rena@actualize.co"
      ...
      Lead.joins(:events).where("events.name = 'Tour'").where(appointment_date: nil).where(connected: false).where("number_of_dials < 2").where(exclude_from_calling: false).where(bad_number: false).count
